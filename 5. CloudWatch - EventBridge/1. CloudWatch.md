...
**Observability là gì?**  
Là khả năng đo lường và hiểu cách hệ thống nội bộ hoạt động nhằm trả lời các câu hỏi liên quan đến hiệu năng, khả năng chịu lỗi, bảo mật và lỗi của hệ thống / ứng dụng.

Để đạt được khả năng quan sát (observability), bạn cần sử dụng Metrics (chỉ số), Logs (nhật ký) và Traces (vết truy dấu). Bạn phải sử dụng chúng cùng nhau; nếu dùng riêng lẻ thì sẽ không đạt được khả năng quan sát thực sự.

**Metrics (Chỉ số)**  
Một giá trị được đo lường trong một khoảng thời gian.  
Ví dụ: Nếu chúng ta đo mức sử dụng CPU và tổng hợp nó theo thời gian, chúng ta có thể có một chỉ số CPU trung bình (Average CPU metric).

**Logs (Nhật ký)**  
Một tệp văn bản trong đó mỗi dòng chứa dữ liệu sự kiện về những gì đã xảy ra tại một thời điểm nhất định.

**Traces (Vết truy dấu)**
Lịch sử của một yêu cầu khi nó đi qua nhiều ứng dụng / dịch vụ khác nhau để chúng ta có thể xác định chính xác hiệu năng hoặc lỗi.
...
Bạn có thể sử dụng bộ lọc sự kiện để lọc ra các log dựa trên Pattern matching syntax (đơn giản hoặc phức tạp)
...
_Ghi chú: Phần lớn khả năng sẵn có của dữ liệu trong các dịch vụ AWS là 1 phút._
...
Một số chỉ số mà bạn có thể nghĩ rằng đã được theo dõi mặc định cho các phiên bản EC2 lại không phải như thế, nó đòi hỏi phải cài đặt CloudWatch Agent.
...
- Page file utilization (xem chú thích bên dưới)
- Log collection (Thu thập log)

_**Page File (hoặc Swap trên Linux)** là vùng trên đĩa mà hệ điều hành dùng như bộ nhớ ảo (virtual memory) khi RAM vật lý đã đầy.  
**Page File Utilization (%)** là tỷ lệ phần trăm dung lượng của Page file hiện đang được sử dụng._

### Cloudwatch Agent

![Image](../images/cloudwatch/cloudwatch-agents.png)

CloudWatch Agent có thể gửi các log đang chạy trên EC2 instance của bạn đến một CloudWatch Log Group.

Để gửi log thì cần:

- Cấu hình của Agent cần được cập nhật để bao gồm các log cần gửi.
- Dịch vụ CloudWatch Agent cần được khởi động lại.

Tệp cấu hình của Agent nằm tại: `/etc/awslogs/awslogs.conf`

Bạn chỉ định vị trí của tệp log và nhóm log (log group) mà bạn muốn gửi log đến. File cấu hình tương tự như sau:
...
Lệnh khởi động lại CloudWatch Agent:
...
CloudWatch Agent có thể được cài đặt bằng AWS Systems Manager (SSM) thông qua Run Command trên EC2 instance mục tiêu.

**Lưu ý:** Bạn phải gán IAM role có tên `CloudWatchAgentServerRole` cho EC2 instance để có thể chạy được agent trên instance đó.
